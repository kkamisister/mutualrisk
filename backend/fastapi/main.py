...
# 2. 공분산
# 2-1. asset_covariance 테이블에서 필요한 데이터 불러오기
sql_cov = "SELECT asset_id_1, asset_id_2, covariance FROM asset_covariance"
...
# 2-2. 모든 자산의 id 목록 가져오기 (df는 기존 자산 테이블 DataFrame)
...
# 2-4. asset_id_1, asset_id_2에 따라 공분산 값들을 채우기
...
async def optimize_portfolio(request: Request):
    ...
    spring_url = data["spring_url"]  # Spring 서버로 전송할 URL
